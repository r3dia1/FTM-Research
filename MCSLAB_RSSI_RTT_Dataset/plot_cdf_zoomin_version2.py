...
BASE_DIR = "." # 用戶原本的設定
OUTPUT_DIR = "cdf_plots_2026_2_4(zoom in version) + geospa-net"
# OUTPUT_DIR = "cdf_plots_2026_3_17(zoom in version)"
...
ARCH_CONFIG = {
    ...
    'FusionDNN': {
        ...
        'subpath': 'FusionDNN/results/all rtt combo/cdf_data_2_4', 
        'label': 'FusionDNN', 
        'color': COLORS[1]
    },
    'DANN': {
    ...
    'FusionDANN': {
        ...
        'subpath': 'FusionDANN/results/all rtt combo/cdf_data_2_4',
        'label': 'FusionDANN', 
        'color': COLORS[3]
    },
    'CDAN': {
        # 'subpath': 'FusionCDAN/ablation test(s:t=1:0.5) 2-4/CDAN/results/cdf_data',
        'subpath': 'CDAN/results/all rtt combo/cdf_data_2_4',
        'label': 'CDAN', 
        'color': COLORS[4]
    },
    'FusionCDAN': {
        # 'subpath': 'FusionCDAN/ablation test(s:t=1:0.5) 2-4/FusionCDAN/results/cdf_data(64)',
        'subpath': 'FusionCDAN final/results/all rtt combo/cdf_data_2_4',
        'label': 'FusionCDAN', 
        'color': COLORS[5]
    },
    'DAFI': {
        # 'subpath': 'DAFI/results/cdf_data_2_4(1:0.5)',
        'subpath': 'DAFI/results/all rtt combo/cdf_data_2_4',
        'label': 'DAFI', 
        'color': COLORS[6]
    },
    ...
    'GACDAN': {
        ...
        'subpath': 'DuGDA/results/cdf_data_2_4(400)',
        'label': 'DuGDA', 
        'color': DUGDA_COLOR
    },
    'GeoSPA-Net': {
...
#     },
#     'FusionCDAN': {
#         'subpath': 'FusionCDAN/ablation test(s:t=1:0.5) 3-17/FusionCDAN/results/cdf_data',  # 請調整路徑
#         'label': 'FusionCDAN', 
#         'color': COLORS[5]
#     },
#     'DAFI': {
...
def parse_filename(filename):
    ...
    if "Only_RTT" in name or "Only_RSSI" in name or "OnlyRTT" in name:
        return None
...
def main():
    ...
    for arch_key, config in ARCH_CONFIG.items():
        ...
        for f in files:
            # 1. 關鍵修復：先透過 parse_filename 擋掉 Only_RTT 與 Only_RSSI
            rtt_count = parse_filename(f)
            if rtt_count is None:
                continue  # 只要是 None 就直接跳過，不往下執行
                
            # 2. 確定是純 Fusion 數據後，再來抽取組合字串
            name = os.path.basename(f).replace(".npy", "")
            name_no_seed = re.sub(r'_seed\d+$', '', name)
            parts = name_no_seed.split('_')
            rtt_list = [p for p in parts if p.isdigit()]
            combo_str = "_".join(rtt_list) # 變成 "1_2_4"
            
            label_key = config['label']
            if rtt_count not in plot_data:
                plot_data[rtt_count] = {}
            if label_key not in plot_data[rtt_count]:
                # combo_dict 用來把相同的組合放在一起
                plot_data[rtt_count][label_key] = {'data': [], 'combo_dict': {}, 'arch': arch_key}
            
            try:
                arr = np.load(f)
                plot_data[rtt_count][label_key]['data'].extend(arr) # 畫圖用的總資料
                
                # 將數據歸類到對應的組合中
                if combo_str not in plot_data[rtt_count][label_key]['combo_dict']:
                    plot_data[rtt_count][label_key]['combo_dict'][combo_str] = []
                plot_data[rtt_count][label_key]['combo_dict'][combo_str].extend(arr)
    ...
    for count in sorted_counts:
        ...
        axins = inset_axes(ax, width="40%", height="40%", loc="lower left", 
                           bbox_to_anchor=(0.575, 0.5, 1, 1), bbox_transform=ax.transAxes)

        # ---------------------------------------------------------
        # 修改點 2：先算各組合的平均，再算這些平均的標準差
        # ---------------------------------------------------------
        for label in sorted_labels:
            ...
            # 算出每一個「獨立組合」的最終平均值
            combo_means = []
            for combo_str, combo_data in info['combo_dict'].items():
                combo_means.append(np.mean(combo_data))
            ...
            # 使用 ddof=1 計算組合間的標準差 (如果只有1個組合，結果會是 0 或 nan，可以用 np.nan_to_num 處理)
            combo_std = np.std(combo_means, ddof=1) if len(combo_means) > 1 else 0.0
    
            print(f"  > {label:<12} | Combos: {len(combo_means):<2} | Overall Mean: {mean_error:.4f}m | True Combo STD: {combo_std:.4f}m")
            ...
            linewidth = 2
            linestyle = '-'
            if label == 'DuGDA' or label == 'GeoSPA-Net':
                linewidth = 4  # 最粗
                linestyle = '-' # 實線
            elif label == 'DNN':
                linestyle = '--' # 虛線
        ...
        # 【修正 3】建議將 dpi 改為 300 或 600 以加快存檔速度並保持合理檔案大小
        # bbox_inches='tight' 已經能很好地裁切邊緣，不需要 tight_layout
