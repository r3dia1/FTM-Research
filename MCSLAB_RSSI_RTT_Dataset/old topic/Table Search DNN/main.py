import torch
import torch.nn as nn
import torch.optim as optim
import numpy as np
import math
import copy
from data_utils import get_data_loaders

# === 路徑設定 ===
# 請確保路徑正確
STUDENT_CSV = '../2025_1_3/mcslab_2025_1_3.csv'
TEACHER_POS3 = '../2026_1_1/Server_Wide_20260101_065323_location_AP3.csv'
TEACHER_POS2 = '../2026_1_1/Server_Wide_20260101_124356_location_AP2.csv'
TEACHER_POS1 = '../2026_1_1/Server_Wide_20260101_075453_location_AP1.csv'
TEACHER_POS4 = '../2026_1_1/Server_Wide_20260101_113813_location_AP4.csv'

# === 設定區 ===
TEACHER_CONFIGS = {
    'Teacher_RTT_1': {'file': TEACHER_POS1},
    'Teacher_RTT_2': {'file': TEACHER_POS2},
    'Teacher_RTT_3': {'file': TEACHER_POS3},
    'Teacher_RTT_4': {'file': TEACHER_POS4}
}

FEATURE_CONFIG = {
    'rssi_cols': ['RSSI_1', 'RSSI_2', 'RSSI_3', 'RSSI_4']
}

# === [重要參數] 資料量控制 ===
# 若設為 100，則每個座標點只會隨機抽出 100 筆資料進入後續流程
# 若設為 None，則使用所有資料
SAMPLES_PER_RP = 50  

# === 其他超參數 ===
K_NEIGHBORS = 35      
BATCH_SIZE = 32
LEARNING_RATE = 0.001
MAX_EPOCHS = 1000
PATIENCE = 300
NUM_RUNS = 5 

# ... (LABEL_TO_COORDS, DNN, EarlyStopping, calculate_distance 保持不變) ...
# 為節省版面，請保留原來的 DNN class 定義、EarlyStopping class 定義等

LABEL_TO_COORDS = {
    "1-1": (0, 0), "1-2": (0.6, 0), "1-3": (1.2, 0), "1-4": (1.8, 0), "1-5": (2.4, 0), "1-6": (3.0, 0),"1-7": (3.6, 0), "1-8": (4.2, 0), "1-9": (4.8, 0), "1-10": (5.4, 0), "1-11": (6.0, 0),
    "2-1": (0, 0.6), "2-11": (6.0, 0.6),
    "3-1": (0, 1.2), "3-11": (6.0, 1.2),
    "4-1": (0, 1.8), "4-11": (6.0, 1.8),
    "5-1": (0, 2.4), "5-11": (6.0, 2.4),
    "6-1": (0, 3.0), "6-2": (0.6, 3.0), "6-3": (1.2, 3.0), "6-4": (1.8, 3.0), "6-5": (2.4, 3.0),"6-6": (3.0, 3.0), "6-7": (3.6, 3.0), "6-8": (4.2, 3.0), "6-9": (4.8, 3.0), "6-10": (5.4, 3.0), "6-11": (6.0, 3.0),
    "7-1": (0, 3.6), "7-11": (6.0, 3.6),
    "8-1": (0, 4.2), "8-11": (6.0, 4.2),
    "9-1": (0, 4.8), "9-11": (6.0, 4.8),
    "10-1": (0, 5.4), "10-11": (6.0, 5.4),
    "11-1": (0, 6.0), "11-2": (0.6, 6.0), "11-3": (1.2, 6.0), "11-4": (1.8, 6.0), "11-5": (2.4, 6.0),"11-6": (3.0, 6.0), "11-7": (3.6, 6.0), "11-8": (4.2, 6.0), "11-9": (4.8, 6.0), "11-10": (5.4, 6.0), "11-11": (6.0, 6.0)
}

class DNN(nn.Module):
    def __init__(self, input_dim, num_classes):
        super(DNN, self).__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, 128),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(256, 128),
            nn.BatchNorm1d(128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, num_classes)
        )
    def forward(self, x):
        return self.net(x)

class EarlyStopping:
    def __init__(self, patience=10, verbose=False, path='checkpoint.pt'):
        self.patience = patience
        self.verbose = verbose
        self.counter = 0
        self.best_loss = None
        self.early_stop = False
        self.best_model_wts = None
        self.path = path
    def __call__(self, val_loss, model):
        if self.best_loss is None:
            self.best_loss = val_loss
            self.save_checkpoint(val_loss, model)
        elif val_loss > self.best_loss:
            self.counter += 1
            if self.counter >= self.patience:
                self.early_stop = True
        else:
            self.best_loss = val_loss
            self.save_checkpoint(val_loss, model)
            self.counter = 0
    def save_checkpoint(self, val_loss, model):
        self.best_model_wts = copy.deepcopy(model.state_dict())

def calculate_distance(label_str_1, label_str_2):
    p1 = LABEL_TO_COORDS.get(label_str_1, (0, 0))
    p2 = LABEL_TO_COORDS.get(label_str_2, (0, 0))
    return math.sqrt((p1[0] - p2[0])**2 + (p1[1] - p2[1])**2)

def run_single_experiment(run_id):
    print(f"\n[Run {run_id}] Setting up data...")
    
    # 傳入 SAMPLES_PER_RP
    data = get_data_loaders(
        student_csv=STUDENT_CSV,
        teacher_configs=TEACHER_CONFIGS,
        feature_config=FEATURE_CONFIG,
        batch_size=BATCH_SIZE,
        random_state=42 + run_id * 100,
        k_neighbors=K_NEIGHBORS,
        samples_per_label=SAMPLES_PER_RP  # New Parameter
    )
    
    if data is None:
        print("Error: Data setup failed.")
        return None
        
    loaders, meta = data
    train_loader = loaders['train']
    val_loader = loaders['val']
    test_loader = loaders['test']
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = DNN(meta['input_dim'], meta['num_classes']).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    early_stopping = EarlyStopping(patience=PATIENCE)
    
    print(f"--- Training (Run {run_id}) ---")
    
    for epoch in range(MAX_EPOCHS):
        model.train()
        train_loss = 0.0
        train_correct = 0
        train_total = 0
        
        for inputs, labels in train_loader:
            inputs, labels = inputs.to(device), labels.to(device)
            optimizer.zero_grad()
            outputs = model(inputs)
            loss = criterion(outputs, labels)
            loss.backward()
            optimizer.step()
            train_loss += loss.item()
            _, predicted = torch.max(outputs, 1)
            train_total += labels.size(0)
            train_correct += (predicted == labels).sum().item()
            
        avg_train_loss = train_loss / len(train_loader)
        avg_train_acc = 100 * train_correct / train_total

        model.eval()
        val_loss = 0.0
        val_correct = 0
        val_total = 0
        with torch.no_grad():
            for inputs, labels in val_loader:
                inputs, labels = inputs.to(device), labels.to(device)
                outputs = model(inputs)
                loss = criterion(outputs, labels)
                val_loss += loss.item()
                _, predicted = torch.max(outputs, 1)
                val_total += labels.size(0)
                val_correct += (predicted == labels).sum().item()
        
        avg_val_loss = val_loss / len(val_loader)
        avg_val_acc = 100 * val_correct / val_total
        
        early_stopping(avg_val_loss, model)
        
        if (epoch + 1) % 20 == 0:
            print(f"Epoch {epoch+1} | Train Acc: {avg_train_acc:.2f}% | Val Acc: {avg_val_acc:.2f}%")
            
        if early_stopping.early_stop:
            print("Early stopping.")
            break
            
    model.load_state_dict(early_stopping.best_model_wts)
    model.eval()
    correct = 0
    total = 0
    mde_list = []
    
    with torch.no_grad():
        for inputs, labels in test_loader:
            inputs, labels = inputs.to(device), labels.to(device)
            outputs = model(inputs)
            _, predicted = torch.max(outputs, 1)
            total += labels.size(0)
            correct += (predicted == labels).sum().item()
            
            p_strs = meta['label_encoder'].inverse_transform(predicted.cpu().numpy())
            t_strs = meta['label_encoder'].inverse_transform(labels.cpu().numpy())
            for p, t in zip(p_strs, t_strs):
                mde_list.append(calculate_distance(p, t))
                
    acc = 100 * correct / total
    mean_mde = np.mean(mde_list)
    print(f"Run {run_id} Result -> Test Acc: {acc:.2f}%, MDE: {mean_mde:.4f}m")
    return acc, mean_mde

def main():
    accs = []
    mdes = []
    for i in range(NUM_RUNS):
        res = run_single_experiment(i)
        if res:
            accs.append(res[0])
            mdes.append(res[1])
            
    print("\n=== Final ===")
    print(f"Avg Acc: {np.mean(accs):.2f}%")
    print(f"Avg MDE: {np.mean(mdes):.4f}m")

if __name__ == "__main__":
    main()