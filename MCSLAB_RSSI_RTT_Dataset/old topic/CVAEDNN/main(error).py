...
import os
import copy
from data_utils import get_data_loaders  # 確保這引用的是剛修改過的 data_utils.py
...
CVAE_CONFIG = {
    'Dist_Pred_1': {'train_file': TEACHER_POS1},
    'Dist_Pred_2': {'train_file': TEACHER_POS2},
    'Dist_Pred_3': {'train_file': TEACHER_POS3},
    'Dist_Pred_4': {'train_file': TEACHER_POS4}
}

# 定義這一次 DNN 實驗要使用的輸入特徵
# 邏輯：Real RSSI (Student) + Generated RTT (Teacher Profile)
SELECTED_FEATURES = [
    # 1. 環境特徵 (Student 真實感受到的訊號強度)
    'RSSI_1', 'RSSI_2', 'RSSI_3', 'RSSI_4', 
    
    # 2. 虛擬測距特徵 (CVAE 根據 Label 生成的理想 Teacher RTT)
    'Dist_Pred_1', 
    'Dist_Pred_2', 
    'Dist_Pred_3', 
    'Dist_Pred_4'
    
    # 注意：這裡 *不包含* 'Dist_mm_X' (Student 真實 RTT)
]
...
def run_single_experiment(run_id):
    ...
    data = get_data_loaders(
        ...
        coords_map=LABEL_TO_COORDS,  # 傳入座標表供 CVAE 使用
