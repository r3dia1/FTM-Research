...
class DAFIModel(nn.Module):
    def __init__(self, input_dim, num_classes):
        ...
        # # 特徵提取器: 3個基礎區塊，維度依序為 360, 480, 600 [cite: 244]
        # self.feature_extractor = nn.Sequential(
        #     make_block(input_dim, 360),
        #     make_block(360, 480),
        #     make_block(480, 600)
        # )

        # # 位置分類器: 2個基礎區塊 (300, 100) + 最終輸出層 [cite: 255, 256]
        # self.class_classifier = nn.Sequential(
        #     make_block(600, 300),
        #     make_block(300, 100),
        ...
        # 領域分類器 1: 2個基礎區塊 (300, 100) + 最終輸出層 [cite: 271, 272]
        ...
        # 領域分類器 2: 2個基礎區塊 (300, 100) + 最終輸出層 [cite: 271, 272]
        self.domain_classifier2 = nn.Sequential(
            make_block(128 + num_classes, 300),
            make_block(300, 100),
            nn.Linear(100, 2)
        )
...
def calc_dafi_losses(model, s_data, s_label, t_data, alpha, num_classes, device, margin=0.1):
    ...
    # (1) Source Classification (SC)
    loss_sc = F.cross_entropy(s_logits, s_label)
    
    # (2) Domain Classification (DC = DC1 + DC2)
    ...
    # (3) Target Classification (TC: Entropy Minimization)
    loss_tc = -torch.mean(torch.sum(t_softmax * torch.log(t_softmax + 1e-5), dim=1))

    # (4) Class Alignment (CA)
    with torch.no_grad():
    ...
    for c in range(num_classes):
        u_as = s_feat[s_label == c]          # Anchor
        u_ps = t_feat[pseudo_label_t == c]   # Positive
        u_ns = t_feat[pseudo_label_t != c]   # Negative

        if u_as.size(0) > 0 and u_ps.size(0) > 0 and u_ns.size(0) > 0:
            # 移除 .pow(2)，嚴格遵循 L2-norm 距離計算 
            dist_ap = torch.cdist(u_as, u_ps, p=2) 
            dist_an = torch.cdist(u_as, u_ns, p=2)
    ...
    # 總損失 (依論文 Sec 5.2.5, 所有 lambda 設為 1，移除 2.0*loss_dc) 
...
def main():
    ...
    for seed in seed_candidate:
        ...
        if has_future_test:
            ...
            np.save(os.path.join(CDF_DIR, f"error_{COMBO_NAME}_future_seed{seed}.npy"), f_errors)
        else:
            print(f"Seed {seed} | Src Acc: {s_acc:.4f} | Src MDE: {s_mde:.4f} | Tgt Acc: {t_acc:.4f} | Tgt MDE: {t_mde:.4f}")
