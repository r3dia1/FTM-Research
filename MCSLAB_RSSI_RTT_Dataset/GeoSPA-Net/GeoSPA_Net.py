# ===================== Version Info =============================
# Architecture: GeoSPA-Net (Extension of DuGDA)
# Modifications based on the paper draft:
# 1. Stage 1: Physical Bias Decoupling (Diff_RSSI directly used)
# 2. Stage 2: PCN for Software-Defined Densification (Virtual Anchors)
# 3. Stage 3: GA-CDAN with Modality-Isolation & Uncertainty Gating
# ================================================================
...
# 接收如 "1 2 4" 這樣的字串，代表 mcAPs (具備 FTM 的真實錨點)
...
print(f"==========================================")
print(f"Current Architecture: GeoSPA-Net")
...
# ==========================================
# 2. [新增] Stage 2: PCN 與 Densification
# ==========================================
class PathLossCalibrationNetwork(nn.Module):
    """
    PCN: 接收 Diff_RSSI (Context)，輸出 4 個 AP 的估計距離。
    訓練時僅對 mcAP 計算 Huber Loss。
    """
    def __init__(self, in_dim=6, out_dim=4):
...
def train_pcn(source_loader, mc_idx, device, epochs=50):
    """獨立訓練 PCN 網路"""
    pcn = PathLossCalibrationNetwork().to(device)
    # 若該組合沒有 mcAP (全部都是 Legacy)，則直接返回未初始化的網路 (避免報錯，雖然實務上不會發生)
    if len(mc_idx) == 0:
        return pcn

    optimizer = optim.Adam(pcn.parameters(), lr=0.01, weight_decay=1e-4)
    # 使用 Huber Loss 抵抗多路徑與 NLOS 異常值
    criterion = nn.HuberLoss(delta=0.5)
    
    pcn.train()
    print(">> Training PCN (Stage 2) on Source Domain...")
    for epoch in range(epochs):
        total_loss = 0
        for rssi, rtt_gt, _ in source_loader:
            rssi, rtt_gt = rssi.to(device), rtt_gt.to(device)
            optimizer.zero_grad()
            
            pred_rtt = pcn(rssi)
            # 僅針對具有真實標籤的 mcAP 計算 Loss
    ...
    print(f">> PCN Training Finished. Final Loss: {total_loss/len(source_loader):.4f}")
    pcn.eval() # 凍結 PCN
    return pcn
...
def apply_densification_and_uncertainty(rssi, rtt_real, pcn, mc_idx, legacy_idx, device):
    """
    整合 Stage 2 推論：
    1. 生成 Densified RTT (\tilde{d})
    2. 生成 Uncertainty Vector (\sigma)
    """
    ...
    # mcAPs: 使用真實 RTT，不確定性低
    if len(mc_idx) > 0:
        sigma[:, mc_idx] = 0.1
        
    # Legacy APs: 填入 PCN 虛擬預測值，不確定性高
    if len(legacy_idx) > 0:
...
class GeoSPANet(nn.Module):
    def __init__(self, num_classes=49, hidden_dim=64):
        ...
        # RSSI Branch Input: 6 (Diff) + 4 (Sigma) = 10
        ...
        # RTT Branch Input: 4 (Densified RTT) + 4 (Sigma) = 8
        ...
        # GA-CDAN 判別器 (Condition = Classes + 2 Coords)
    ...
    def forward(self, rssi, rtt_densified, sigma, coord_tensor, alpha=1.0):
        # Uncertainty Gating (Concatenation)
        # x_rssi = torch.cat([rssi, sigma], dim=1) # 為甚麼這裡要加上標準差?
        # x_rtt = torch.cat([rtt_densified, sigma], dim=1)
        x_rssi = rssi
        x_rtt = rtt_densified

        f_rssi = self.rssi_extractor(x_rssi)
        f_rtt = self.rtt_extractor(x_rtt)
        
        f_cat = torch.cat((f_rssi, f_rtt), dim=1)
        class_logits = self.class_classifier(f_cat)
        softmax_output = F.softmax(class_logits, dim=1)

        # Expected Coordinate Formulation (Stage 3)
...
def load_raw_data(csv_path):
    """GeoSPA-Net: 永遠載入全部 4 個 RTT，以便做 PCN Densification"""
...
# 評估與驗證流程 (需套用 PCN)
# ==========================================
def validate_process(model, pcn, source_val_loader, target_val_loader, device, coord_tensor):
    model.eval()
    ...
    with torch.no_grad():
        for s_rssi, s_rtt, s_label in source_val_loader:
            ...
            s_rtt_den, s_sig = apply_densification_and_uncertainty(s_rssi, s_rtt, pcn, MC_IDX, LEGACY_IDX, device)
            class_out_s, _, _, _ = model(s_rssi, s_rtt_den, s_sig, coord_tensor, alpha=0)
        ...
        for t_rssi, t_rtt, _ in target_val_loader:
            ...
            t_rtt_den, t_sig = apply_densification_and_uncertainty(t_rssi, t_rtt, pcn, MC_IDX, LEGACY_IDX, device)
            _, _, _, softmax_t = model(t_rssi, t_rtt_den, t_sig, coord_tensor, alpha=0)
...
def evaluate_test(model, pcn, data_loader, coord_tensor, device, return_all_errors=False):
    ...
    with torch.no_grad():
        for rssi, rtt, labels in data_loader:
            ...
            class_out, _, _, _ = model(rssi, rtt_den, sig, coord_tensor, alpha=0)
...
def main():
    ...
    for seed in seed_candidate:
        ...
        # 1. 訓練 PCN (Stage 2)
        pcn_model = train_pcn(source_loader, MC_IDX, DEVICE, epochs=50)

        # 2. 初始化 GeoSPA-Net (Stage 3)
        ...
        for epoch in range(num_epochs):
            ...
            for (s_rssi_b, s_rtt_b, s_lbl_b), (t_rssi_b, t_rtt_b, _) in zip(source_loader, target_train_loader):
                ...
                # 套用 Stage 2 Densification 與 Uncertainty
                s_rtt_den, s_sig = apply_densification_and_uncertainty(s_rssi_b, s_rtt_b, pcn_model, MC_IDX, LEGACY_IDX, DEVICE)
                t_rtt_den, t_sig = apply_densification_and_uncertainty(t_rssi_b, t_rtt_b, pcn_model, MC_IDX, LEGACY_IDX, DEVICE)
                
                # Forward Pass
                cls_out_s, d_logits_rssi_s, d_logits_rtt_s, softmax_s = model(s_rssi_b, s_rtt_den, s_sig, COORD_TENSOR, alpha=alpha)
                _, d_logits_rssi_t, d_logits_rtt_t, softmax_t = model(t_rssi_b, t_rtt_den, t_sig, COORD_TENSOR, alpha=alpha)
                
                # Spatial Loss (Stage 3)
            ...
            val_s_acc, val_t_entropy = validate_process(model, pcn_model, source_val_loader, target_val_loader, DEVICE, COORD_TENSOR)
            ...
            if (epoch + 1) % 10 == 0:
                t_acc, t_mde, _ = evaluate_test(model, pcn_model, target_test_loader, COORD_TENSOR, DEVICE)
        ...
        t_acc, t_mde, t_errors = evaluate_test(model, pcn_model, target_test_loader, COORD_TENSOR, DEVICE, return_all_errors=True)
        s_acc, s_mde, s_errors = evaluate_test(model, pcn_model, source_test_loader, COORD_TENSOR, DEVICE, return_all_errors=True)
