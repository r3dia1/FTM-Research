# ===================== Version Info =============================
# Architecture: GeoSPA-Net (Consistency & Adaptive Uncertainty)
# Modifications:
# 1. train_pcn() now includes consistency regularization for legacy APs.
# 2. apply_densification_and_uncertainty() computes sample-wise uncertainty 
#    based on source-target prediction disagreement.
# 3. Main architecture (GeoSPANet) completely untouched.
# ================================================================
...
print(f"==========================================")
print(f"Current Architecture: GeoSPA-Net (Consistency & Adaptive Uncertainty)")
...
def get_target_centric_features(rssi_batch, target_ap_id):
    ...
    if target_ap_id == 0:
        diff_1 = rssi_batch[:, 4].unsqueeze(1)
        diff_2 = rssi_batch[:, 5].unsqueeze(1)
        diff_3 = rssi_batch[:, 6].unsqueeze(1)
    elif target_ap_id == 1:
        diff_1 = rssi_batch[:, 10].unsqueeze(1)
        diff_2 = rssi_batch[:, 7].unsqueeze(1)
        diff_3 = rssi_batch[:, 8].unsqueeze(1)
    elif target_ap_id == 2:
        diff_1 = rssi_batch[:, 11].unsqueeze(1)
        diff_2 = rssi_batch[:, 13].unsqueeze(1)
        diff_3 = rssi_batch[:, 9].unsqueeze(1)
    elif target_ap_id == 3:
        diff_1 = rssi_batch[:, 12].unsqueeze(1)
        diff_2 = rssi_batch[:, 14].unsqueeze(1)
        diff_3 = rssi_batch[:, 15].unsqueeze(1)
    else:
...
# [NEW] 修改後的 train_pcn，加入 consistency regularization
def train_pcn(data_loader, train_ap_indices, device, epochs=50, domain_name="Source", 
              pretrained_model=None, source_pcn=None, legacy_idx=None, consistency_lambda=0.1):
    ...
    if source_pcn is not None:
        source_pcn.eval()

    optimizer = optim.Adam(pcn.parameters(), lr=0.01, weight_decay=1e-4)
    criterion_sup = nn.HuberLoss(delta=0.05) 
    ...
    for epoch in range(epochs):
        ...
        for rssi, rtt_gt, rtt_mask, _ in data_loader:
            ...
            # 1. Supervised mcAP loss
            for ap_id in train_ap_indices:
                features = get_target_centric_features(rssi, ap_id)
                y_true = rtt_gt[:, ap_id]
                valid_mask = rtt_mask[:, ap_id]
                
                pred_dist = pcn(features, ap_id).squeeze()
                
                if valid_mask.sum() > 0:
                    loss_sup = criterion_sup(pred_dist[valid_mask], y_true[valid_mask])
                    batch_loss += loss_sup
            
            # 2. Consistency regularization for legacy APs
            if source_pcn is not None and legacy_idx is not None:
                for ap_id in legacy_idx:
                    features = get_target_centric_features(rssi, ap_id)
                    
                    with torch.no_grad():
                        pred_source = source_pcn(features, ap_id).squeeze()
                    
                    pred_target = pcn(features, ap_id).squeeze()
                    
                    # Smooth adaptation constraint
                    loss_cons = F.mse_loss(pred_target, pred_source)
                    batch_loss += consistency_lambda * loss_cons
    ...
    print(f">> {domain_name} PCN Training Finished. Avg Loss: {total_loss/len(data_loader):.4f}")
    pcn.eval()
    return pcn

# [NEW] 修改後的 apply_densification_and_uncertainty，加入 adaptive uncertainty
def apply_densification_and_uncertainty(rssi, rtt_real, pcn_target, pcn_source, mc_idx, legacy_idx, device):
    ...
    # 2. Legacy AP: hallucinated RTT & adaptive uncertainty
    with torch.no_grad():
        if len(legacy_idx) > 0:
            for ap_id in legacy_idx:
                features = get_target_centric_features(rssi, ap_id)
                pred_target = pcn_target(features, ap_id).squeeze()
                
                rtt_densified[:, ap_id] = pred_target
                
                # Uncertainty from source-target disagreement
                if pcn_source is not None:
                    pred_source = pcn_source(features, ap_id).squeeze()
                    disagreement = torch.abs(pred_target - pred_source)
                    sigma[:, ap_id] = torch.clamp(disagreement, min=0.1, max=2.0)
                else:
                    # Fallback for source domain itself
                    sigma[:, ap_id] = 1.0 
                    
    # [備註] Uncertainty gating 動作維持在 GeoSPANet.forward 中執行 (rtt_gated = rtt_densified * exp(-sigma))
    return rtt_densified, sigma

# ==========================================
# 3. Stage 3 模型架構：GeoSPA-Net (完全維持原樣)
# ==========================================
...
def validate_process(model, pcn_s, pcn_t, source_val_loader, target_val_loader, device, coord_tensor):
    ...
    with torch.no_grad():
        for s_rssi, s_rtt, s_mask, s_label in source_val_loader:
            ...
            # Source 驗證時，不使用 pcn_source 進行 disagreement 計算
            s_rtt_den, s_sig = apply_densification_and_uncertainty(s_rssi, s_rtt, pcn_s, None, MC_IDX, LEGACY_IDX, device)
        ...
        for t_rssi, t_rtt, t_mask, _ in target_val_loader:
            ...
            # Target 驗證時傳入 pcn_s 進行 uncertainty 估算
            t_rtt_den, t_sig = apply_densification_and_uncertainty(t_rssi, t_rtt, pcn_t, pcn_s, MC_IDX, LEGACY_IDX, device)
...
def evaluate_test(model, pcn_target, data_loader, coord_tensor, device, pcn_source_for_uncertainty=None, return_all_errors=False):
    model.eval()
    ...
    with torch.no_grad():
        for rssi, rtt, mask, labels in data_loader:
            ...
            rtt_den, sig = apply_densification_and_uncertainty(rssi, rtt, pcn_target, pcn_source_for_uncertainty, MC_IDX, LEGACY_IDX, device)
...
def main():
    ...
    for seed in seed_candidate:
        ...
        # ==========================================
        ALL_APS_IDX = list(range(TOTAL_APS)) 

        # 1. Source Domain：擁有全部真實數據，訓練所有的 Head (無 legacy 概念)
        pcn_source = train_pcn(source_loader, ALL_APS_IDX, DEVICE, epochs=50, domain_name="Source")

        # 2. Target Domain：建立新模型並繼承 Source 的權重
        ...
        # 3. Target Domain 微調：傳入 pcn_source 與 legacy_idx 來計算 Consistency Regularization
        pcn_target = train_pcn(
            data_loader=target_train_loader, 
            train_ap_indices=MC_IDX, 
            device=DEVICE, 
            epochs=50, 
            domain_name="Target", 
            pretrained_model=pcn_target_init,
            source_pcn=pcn_source,           # [NEW]
            legacy_idx=LEGACY_IDX,           # [NEW]
            consistency_lambda=0.1           # [NEW] 建議起點 0.1
        )
        ...
        for epoch in range(num_epochs):
            ...
            for (s_rssi_b, s_rtt_b, s_mask_b, s_lbl_b), (t_rssi_b, t_rtt_b, t_mask_b, _) in zip(source_loader, target_train_loader):
                ...
                # Source 端傳入 pcn_source=None (不需要算 domain shift)
                s_rtt_den, s_sig = apply_densification_and_uncertainty(s_rssi_b, s_rtt_b, pcn_source, None, MC_IDX, LEGACY_IDX, DEVICE)
                # Target 端傳入 pcn_source 協助計算 uncertainty
                t_rtt_den, t_sig = apply_densification_and_uncertainty(t_rssi_b, t_rtt_b, pcn_target, pcn_source, MC_IDX, LEGACY_IDX, DEVICE)
            ...
            if (epoch + 1) % 10 == 0:
                t_acc, t_mde, _ = evaluate_test(model, pcn_target, target_test_loader, COORD_TENSOR, DEVICE, pcn_source_for_uncertainty=pcn_source)
        ...
        t_acc, t_mde, t_errors = evaluate_test(model, pcn_target, target_test_loader, COORD_TENSOR, DEVICE, pcn_source_for_uncertainty=pcn_source, return_all_errors=True)
        s_acc, s_mde, s_errors = evaluate_test(model, pcn_source, source_test_loader, COORD_TENSOR, DEVICE, pcn_source_for_uncertainty=None, return_all_errors=True)
