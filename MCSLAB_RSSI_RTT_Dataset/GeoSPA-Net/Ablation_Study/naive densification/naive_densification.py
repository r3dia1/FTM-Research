...
def validate_process(model, pcn_s, pcn_t, source_val_loader, target_val_loader, device, coord_tensor):
    ...
    with torch.no_grad():
        for s_rssi, s_rtt, s_label in source_val_loader:
            ...
            class_out_s, _, _, _ = model(s_diff_rssi, s_rtt_den, coord_tensor, alpha=0)
        ...
        for t_rssi, t_rtt, _ in target_val_loader:
            ...
            _, _, _, softmax_t = model(t_diff_rssi, t_rtt_den, coord_tensor, alpha=0)
...
def evaluate_test(model, pcn, data_loader, coord_tensor, device, return_all_errors=False):
    ...
    with torch.no_grad():
        for rssi, rtt, labels in data_loader:
            ...
            diff_rssi = rssi[:, 4:]
            class_out, _, _, _ = model(diff_rssi, rtt_den, coord_tensor, alpha=0)
    ...
    for seed in seed_candidate:
        ...
        for epoch in range(num_epochs):
            ...
            for (s_rssi_b, s_rtt_b, s_lbl_b), (t_rssi_b, t_rtt_b, _) in zip(source_loader, target_train_loader):
                ...
                cls_out_s, d_logits_rssi_s, d_logits_rtt_s, softmax_s = model(s_diff_rssi, s_rtt_den, COORD_TENSOR, alpha=alpha)
                _, d_logits_rssi_t, d_logits_rtt_t, softmax_t = model(t_diff_rssi, t_rtt_den, COORD_TENSOR, alpha=alpha)
