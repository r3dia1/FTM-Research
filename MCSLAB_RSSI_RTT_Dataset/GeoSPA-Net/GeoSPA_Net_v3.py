...
# 新增一個輔助函數，用來動態萃取「以目標 AP 為中心」的 4 維特徵
def get_target_centric_features(rssi_batch, target_ap_id):
    # rssi_batch 包含了前 4 維的絕對 RSSI，我們只需取前 4 維來重組
    abs_rssi = rssi_batch[:, :4] 
    
    # 1. 取出 Target AP 的絕對 RSSI: 維度變成 [batch_size, 1]
    target_rssi = abs_rssi[:, target_ap_id].unsqueeze(1) 
    
    # 2. 找出其他 3 台 AP 的 index
    other_indices = [i for i in range(TOTAL_APS) if i != target_ap_id]
    
    # 3. 取出其他 3 台 AP 的絕對 RSSI: 維度變成 [batch_size, 3]
    other_rssi = abs_rssi[:, other_indices]
    
    # 4. 計算 Target 與其他 AP 的差分 RSSI (保留空間相對關係)
    diff_rssi = target_rssi - other_rssi # Broadcasting: [batch_size, 3]
    
    # 5. 拼接成 4 維輸入特徵 [Target_RSSI, Diff_1, Diff_2, Diff_3]
    features = torch.cat([target_rssi, diff_rssi], dim=1) 
    return features
