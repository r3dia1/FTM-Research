...
RTT_COMBO_NAME = "DuGDA_" + "_".join(rtt_indices)
...
class DualStreamCDAN(nn.Module):
    ...
    def forward(self, rssi, rtt, coord_tensor, alpha=1.0, use_condition=True):
        """
        coord_tensor: 傳入所有 RP 的真實座標 (C, 2)，用於計算期望座標
        use_condition: 控制是否使用分類器預測作為對齊條件 (Warm-up 用)
        """
        ...
        # --- 修改處：加入條件 Warm-up 控制 ---
        if use_condition:
            # 正常 CDAN：使用預測的機率分佈與期望座標
            expected_coords = torch.mm(softmax_output, coord_tensor) # (B, 2)
            g_cond = torch.cat((softmax_output, expected_coords), dim=1)
        else:
            # Warm-up 階段：使用均勻分佈當作 Dummy Condition，忽略分類器初期的雜訊
            dummy_softmax = torch.ones_like(softmax_output) / self.num_classes
            dummy_coords = torch.mm(dummy_softmax, coord_tensor)
            g_cond = torch.cat((dummy_softmax, dummy_coords), dim=1)
        # ------------------------------------
...
def main():
    ...
    for seed in seed_candidate:
        ...
        WARMUP_EPOCHS = 10
        COND_WARMUP_EPOCHS = 20  # <--- 新增：條件對齊的 Warm-up epoch
        ...
        for epoch in range(num_epochs):
            ...
            use_cond = epoch >= COND_WARMUP_EPOCHS
            ...
            for (s_rssi_b, s_rtt_b, s_lbl_b), (t_rssi_b, t_rtt_b, _) in zip(source_loader, target_train_loader):
                ...
                # --- 修改處：Warmup 期間關閉 Entropy Weighting (CDAN+E) ---
                if use_cond:
                    ...
                    # Warmup 期間，不使用基於分類器的 Entropy 給樣本加權
                    weight_s = torch.ones(s_rssi_b.size(0)).to(DEVICE)
                    weight_t = torch.ones(t_rssi_b.size(0)).to(DEVICE)
                # ----------------------------------------------------------
