...
#         if len(d1) > 0: sns.kdeplot(d1, ax=ax, label=source_date, fill=True, color='blue', alpha=0.3)
#         if len(d2) > 0: sns.kdeplot(d2, ax=ax, label=target_date, fill=True, color='orange', alpha=0.3)
...
#     # 放大主標題，並加上粗體
#     # fig.suptitle(f'Distribution Comparison @ {loc_name}', fontsize=20, fontweight='bold')
...
#         # 加上 linewidth 讓輪廓更明顯，cut=0 防止核密度估計延伸到沒有數據的地方 (例如負的距離)
#         if len(d1) > 0: 
...
#         # 建議：隱藏 Y 軸數字，因為 Density 絕對值通常難以閱讀且不重要
#         ax.set_yticks([]) 
#         ax.set_ylabel('') # 如果連 Density 字眼都不想要，可以直接設為空字串
...
#     plt.tight_layout()
#     # 加上 bbox_inches='tight' 確保大字體標題不會被裁切掉
...
# def plot_kde_comparison(df_single, df_full, loc_name, source_date, target_date, REMOVE_NEGATIVE_DIST=True):
#     """
#     繪製 KDE 分佈比較圖 (修改為上下排列)
#     """
#     # 為了讓 X 軸標籤更完整，直接把單位寫入對應的 list 中
#     features = ['RSSI', 'Dist_mm']
#     x_labels = ['RSSI (dBm)', 'RTT Distance (mm)']
    
#     # 1. 改為上下排列 (2列 1行)，調整 figsize。
#     # 寬度設為 8, 高度設為 10，這樣在論文縮放時比例會比較漂亮
#     fig, axes = plt.subplots(2, 1, figsize=(8, 10))
...
#         # 3. Y 軸處理：保留物理意義 (Probability Density)，但隱藏數字刻度以符合學術排版慣例
#         ax.set_ylabel('Probability Density', fontsize=14)
#         ax.set_yticks([]) 
