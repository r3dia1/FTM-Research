...
def main():
    ...
    print("-" * 95)
    print(f"{'Epoch':<6} | {'Domain Acc':<10} | {'Tot Loss':<10} | {'Cls Loss':<10} | {'D_RSSI':<8} | {'D_RTT':<8} || {'Val Acc':<8} | {'Val MDE':<8} | {'Save'}")
    print("-" * 95)
    ...
    for epoch in range(num_epochs):
        ...
        for (s_rssi_b, s_rtt_b, s_label_b), (t_rssi_b, t_rtt_b) in zip(source_loader, target_train_loader):
            ...
            # 取得預測結果 (logits)
            # d_rssi_s, d_rssi_t 是模型輸出的 logits
            preds_s = torch.argmax(d_rssi_s, dim=1)
            preds_t = torch.argmax(d_rssi_t, dim=1)

            # 計算準確度
            acc_s = (preds_s == 0).float().mean() # Source 應該被預測為 0
            acc_t = (preds_t == 1).float().mean() # Target 應該被預測為 1
        ...
        avg_dom = (W_DOM_RSSI * avg_d_rssi + W_DOM_RTT * avg_d_rtt) / (W_DOM_RSSI + W_DOM_RTT)
        
        # --- Validation (為了找 Best Model) ---
        # 這裡我們用 Target Test Set 來評估 MDE，以決定是否儲存模型
        ...
        print(f"{epoch+1:<6} | {avg_domain_acc:<10.4f} | {avg_total:<10.4f} | {avg_cls:<10.4f} | {avg_d_rssi:<8.4f} | {avg_d_rtt:<8.4f} || {val_acc:<8.2f} | {val_mde:<8.4f} | {save_mark}")
