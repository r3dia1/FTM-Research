...
def main():
    ...
    for seed in seed_candidate:
        ...
        W_SCORE_DOM_1 = 0.1        # 分數權重：域損失 (加分項，設小一點以保守起見)
        W_SCORE_DOM_2 = 0.3
        ...
        for epoch in range(num_epochs):
            ...
            # Score = (Dom_RSSI + Dom_RTT) * 0.1 - (Cls_Loss) * 1.0 - (Dom_diff) * 0.3
            # 6 RSSI_DIff + 4 RTT:  0.7640m / 0.8163m / 0.7197m
            # 4 RSSI + 4 RTT:       0.9067m / 0.8474m / 0.7947m
            # Score = (Dom_RSSI + Dom_RTT) * 0.3 - (Cls_Loss) * 1.0 - (Dom_diff) * 0.3
            # 6 RSSI_DIFF + 1 RTT:  1.0389m / 1.0847m / 0.9959m (dnn 1.1166m)
            # 4 RSSI + 1 RTT:       1.2539m / 1.3925m / 1.1805m (dnn 1.2909m)
            # -------------------------------------------------------
