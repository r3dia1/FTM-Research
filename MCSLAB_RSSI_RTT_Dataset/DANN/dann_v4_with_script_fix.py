...
def validate_process(model, source_val_loader, target_val_loader, device):
    ...
    # --- 指標 1: Source Accuracy (越高越好) ---
    correct_s = 0
    total_s = 0
    
    # --- 指標 2: Target Entropy (越低越好) ---
    total_entropy_t = 0.0
    num_batches_t = 0
    
    with torch.no_grad():
        # 1. 跑 Source Validation: 計算準確率 (Accuracy)
        # 建議分開跑，不要用 zip，確保完整遍歷 Source 驗證集
        for s_data, s_label in source_val_loader:
            s_data, s_label = s_data.to(device), s_label.to(device)
            
            # Forward pass (DANN 架構)
            class_out, _ = model(s_data, alpha=0) 
            
            # 計算準確數量
            preds = torch.argmax(class_out, dim=1)
            correct_s += (preds == s_label).sum().item()
            total_s += s_data.size(0)

        # 2. 跑 Target Validation: 計算熵 (Entropy)
        # 確保完整遍歷 Target 驗證集
        for t_data, _ in target_val_loader:
            t_data = t_data.to(device)
            
            # Forward pass
            class_out, _ = model(t_data, alpha=0)
            
            # 計算 Softmax 機率分佈
            softmax_out = F.softmax(class_out, dim=1)
            
            # Entropy 公式: H(x) = -sum(p * log(p))
            # 加上 1e-5 避免 log(0)
            entropy = -torch.sum(softmax_out * torch.log(softmax_out + 1e-5), dim=1)
            
            # 累加每個 Batch 的平均 Entropy
            total_entropy_t += entropy.mean().item()
            num_batches_t += 1

    # 計算平均值
    avg_s_acc = (correct_s / total_s) * 100.0 if total_s > 0 else 0.0
    avg_t_entropy = total_entropy_t / num_batches_t if num_batches_t > 0 else 0.0
...
def main():
    ...
    for seed in seed_candidate:
        ...
        for epoch in range(num_epochs):
            ...
            # Validation
            val_s_acc, val_t_entropy = validate_process(model, source_val_loader, target_val_loader, DEVICE)
            current_score = val_s_acc - (5.0 * val_t_entropy)
