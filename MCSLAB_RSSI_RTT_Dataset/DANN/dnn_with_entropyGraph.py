...
import os
import random
import matplotlib.pyplot as plt  # <--- [新增 1] 繪圖庫
...
# ==========================================
# [新增 2] Entropy 計算與繪圖函式
# ==========================================
def calc_and_plot_entropy(model, data_loader, device, seed):
    model.eval()
    entropy_list = []
    ...
    avg_ent = np.mean(entropy_arr)
    std_ent = np.std(entropy_arr)
    
    # 繪製 Histogram
    plt.figure(figsize=(8, 6))
    plt.hist(entropy_arr, bins=30, alpha=0.7, color='skyblue', edgecolor='black')
    plt.title(f'Target Entropy Distribution (Seed {seed})\nMean: {avg_ent:.4f}, Std: {std_ent:.4f}')
    plt.xlabel('Entropy')
    plt.ylabel('Frequency (Number of Samples)')
    plt.grid(True, linestyle='--', alpha=0.5)
    
    # 儲存圖片
    filename = f"entropy_hist_seed_{seed}.png"
    plt.savefig(filename)
    plt.close()
    
    print(f" > Entropy Analysis (Seed {seed}): Mean={avg_ent:.4f}, Std={std_ent:.4f}, Plot saved to {filename}")
    return avg_ent
...
def main():
    ...
    target_acc = []
    target_mde = []
    target_entropy_means = [] # 用來存每個 seed 的平均 entropy
    ...
    for seed in seed_candidate:
        ...
        # [新增 3] 計算並繪製 Entropy
        print("Calculating Entropy for Target Test set...")
        seed_mean_entropy = calc_and_plot_entropy(model, test_loader, DEVICE, seed)
        target_entropy_means.append(seed_mean_entropy)
    ...
    avg_target_ent = np.mean(target_entropy_means) # 統計平均 Entropy

    print(f"source acc: {avg_source_acc:.4f}, source mde: {avg_source_mde:.4f}, target acc: {avg_target_acc:.4f}, target mde: {avg_target_mde:.4f}, target entropy: {avg_target_ent:.4f}")
