...
# === 校正參數設定 (Calibration Config) ===
# Logic: Target = Original + Offset
# 這裡填入針對 Full Set (Student) 的補償值，使其對齊 Teacher
CALIB_CONFIG = {
    'Pos1': {
        'target_bssid': '24:29:34:e2:4c:36',
        'rssi_offset': 2.5,   # 補強弱訊號
        'dist_offset': 0.0    
    },
    'Pos2': {
        'target_bssid': '24:29:34:e1:ef:d4',
        'rssi_offset': 2.1,   
        'dist_offset': -525.0 # 修正 RTT 嚴重偏差
    },
    'Pos3': {
        'target_bssid': 'b0:e4:d5:88:16:86',
        'rssi_offset': 0.0,
        'dist_offset': 60.0   
    },
    'Pos4': {
        'target_bssid': 'e4:5e:1b:a0:5e:85',
        'rssi_offset': 0.0,
        'dist_offset': 40.0
    }
}
...
def get_data_loaders(main_csv_path, selected_features, cvae_config, batch_size=32, random_state=42, samples_per_label=None):
    ...
    # === [新增步驟] 1. 執行資料校正 ===
    # 這會修正 Pos2 的 RTT 偏差和 RSSI 弱訊號
    # df = apply_calibration(df, CALIB_CONFIG)
