...
class FeatureExtractorWithLayerNorm(nn.Module):
    def __init__(self, input_dim, hidden_dim=64):
        super(FeatureExtractorWithLayerNorm, self).__init__()
        
        self.fc1 = nn.Linear(input_dim, 32)
        # 注意：LayerNorm 傳入的是特徵維度 (32)
        self.ln1 = nn.LayerNorm(32) 
        self.relu1 = nn.ReLU(True)
        
        self.fc2 = nn.Linear(32, hidden_dim)
        # 注意：LayerNorm 傳入的是特徵維度 (hidden_dim)
        self.ln2 = nn.LayerNorm(hidden_dim) 
        self.relu2 = nn.ReLU(True)

    def forward(self, x):
        # LayerNorm 不需要 domain_label 了
        x = self.fc1(x)
        x = self.ln1(x)
        x = self.relu1(x)
...
class DualStreamCDAN(nn.Module):
    def __init__(self, num_aps=4, num_classes=49, hidden_dim=64, rtt_input_dim=1):
        ...
        # --- 使用內建 DSBN 的特徵提取器 ---
        self.rtt_extractor = FeatureExtractorWithLayerNorm(rtt_input_dim, hidden_dim)
        self.rssi_extractor = FeatureExtractorWithLayerNorm(6, hidden_dim)
