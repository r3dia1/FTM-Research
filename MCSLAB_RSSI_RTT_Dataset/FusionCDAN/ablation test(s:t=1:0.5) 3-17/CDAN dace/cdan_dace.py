...
class CDAN(nn.Module):
    ...
    def forward(self, data, coord_tensor, alpha=1.0):
...
def validate_process(model, source_val_loader, target_val_loader, device, coord_tensor):
    ...
    with torch.no_grad():
        for (s_data, s_label), (t_data, _) in zip(source_val_loader, target_val_loader):
            ...
            class_out_s, d_logits_s, _ = model(s_data, coord_tensor, alpha=0) 
            _, d_logits_t, _ = model(t_data, coord_tensor, alpha=0)
...
def evaluate_test(model, data_loader, coord_tensor, device, return_all_errors=False):
    ...
    with torch.no_grad():
        for data, labels in data_loader:
            ...
            class_out, _, _ = model(data, coord_tensor, alpha=0)
    ...
    for seed in seed_candidate:
        ...
        for epoch in range(num_epochs):
            ...
            for (s_data_b, s_label_b), (t_data_b,_) in zip(source_loader, target_train_loader):
                ...
                cls_out_s, d_logits_s, softmax_s = model(s_data_b, COORD_TENSOR, alpha=alpha)
                _, d_logits_t, softmax_t = model(t_data_b, COORD_TENSOR, alpha=alpha)
            ...
            # Validation
            val_cls, val_dls = validate_process(model, source_val_loader, target_val_loader, DEVICE, COORD_TENSOR)
