...
def plot_metrics_v7(csv_file_path, output_dir='output_charts'):
    """
    V7 更新:
    ...
    def draw_combined_plot(metric_type):
        ...
        bars = plt.bar(x_pos, values, width=0.5, color=colors[:len(values)])

        # 圖表設定
        plt.title(f'{metric_type.upper()} Performance Comparison (CDAN)', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    # 請確保此路徑下的 CSV 內容如你所述
    csv_files = ["./time_shift_by_date_2_4.csv"]
