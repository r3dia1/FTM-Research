...
RTT_COMBO_NAME = "CDAN_" + "_".join(rtt_indices)
...
class CDAN(nn.Module):
    def __init__(self, num_aps=4, num_classes=49, hidden_dim=64, input_dim=8):
        super(CDAN, self).__init__()
        self.num_classes = num_classes

        # --- 特徵提取器 (動態輸入維度) ---
        self.extractor = nn.Sequential(
            nn.Linear(input_dim, 32), # 修改這裡：根據傳入的維度
            nn.BatchNorm1d(32),
            nn.ReLU(True),
        ...
        self.map = RandomizedMultiLinearMap(hidden_dim, num_classes, output_dim=512)
    ...
    def forward(self, data, alpha=1.0):
        f = self.extractor(data)
        
        class_logits = self.class_classifier(f)
        softmax_output = F.softmax(class_logits, dim=1)

        h = self.map(f, softmax_output)
        h_rev = self.grl(h, alpha)
        d_logits = self.disc(h_rev)

        return class_logits, d_logits, softmax_output
...
def validate_process(model, source_val_loader, target_val_loader, device):
    ...
    with torch.no_grad():
        for (s_data, s_label), (t_data, _) in zip(source_val_loader, target_val_loader):
            ...
            class_out_s, d_logits_s, _ = model(s_data, alpha=0) 
            _, d_logits_t, _ = model(t_data, alpha=0)
            
            loss_cls = F.cross_entropy(class_out_s, s_label, reduction='sum')
            d_label_s = torch.ones(s_data.size(0), 1).to(device)
            d_label_t = torch.zeros(t_data.size(0), 1).to(device)
    ...
    avg_dls = total_dom_loss / num_batches 
    return avg_cls, avg_dls
...
def plot_spatial_tsne(model, data_loader, coord_tensor, device, save_path):
    ...
    使用 PyTorch Hook 取得隱藏層特徵，並根據真實 X/Y 座標進行漸層著色。
    """
    model.eval()
    features = []
    labels_list = []
    
    # 使用 Hook 攔截 extractor 的輸出，完全不動到原模型程式碼
    def hook_fn(module, input, output):
        features.append(output.detach().cpu().numpy())
        
    # 註冊 Hook 到 extractor (特徵提取器)
    handle = model.extractor.register_forward_hook(hook_fn)
    
    with torch.no_grad():
        for data, labels in data_loader:
            data = data.to(device)
            # 觸發 forward，讓 Hook 抓取特徵
            _ = model(data, alpha=0) 
...
def main():
    ...
    for seed in seed_candidate:
        ...
        # 傳入 RTT_INPUT_DIM
        input_dim = RTT_INPUT_DIM + 4
        model = CDAN(num_aps=4, num_classes=len(class_names), input_dim=input_dim).to(DEVICE)
        ...
        print(f"{'Epoch':<6} | {'Total Loss':<10} | {'Tr Cls':<10} | {'Tr Dom':<12} | {'Val Cls':<10} | {'Val Dls':<12} | {'Test MDE':<8}")
        print("-" * 120)
        ...
        for epoch in range(num_epochs):
            ...
            for (s_data_b, s_label_b), (t_data_b,_) in zip(source_loader, target_train_loader):
                s_data_b, s_lbl_b = s_data_b.to(DEVICE), s_label_b.to(DEVICE)
                t_data_b = t_data_b.to(DEVICE)
                
                cls_out_s, d_logits_s, softmax_s = model(s_data_b, alpha=alpha)
                _, d_logits_t, softmax_t = model(t_data_b, alpha=alpha)
                ...
                d_lbl_s = torch.ones(s_data_b.size(0), 1).to(DEVICE)
                d_lbl_t = torch.zeros(t_data_b.size(0), 1).to(DEVICE)
                
                loss_dom_s = domain_criterion(d_logits_s, d_lbl_s)
                loss_dom_t = domain_criterion(d_logits_t, d_lbl_t)
                
                loss_dom = torch.mean(weight_s.view(-1, 1) * loss_dom_s) + torch.mean(weight_t.view(-1, 1) * loss_dom_t)
                
                loss = loss_cls + 1 * loss_dom
                ...
                train_dom_sum += loss_dom.item()
                num_batches += 1
            
            # Validation
            val_cls, val_dls = validate_process(model, source_val_loader, target_val_loader, DEVICE)
            
            save_mark = ""
            current_dom_total = (val_dls - 1.2)
            current_score = (W_SCORE_DOM_1 * current_dom_total) - (W_SCORE_CLS * val_cls)
            ...
            if (epoch + 1) % 1 == 0:
                ...
                print(f"{epoch+1:<6} | {total_loss_sum/num_batches:<10.4f} | {train_cls_sum/num_batches:<10.4f} | {train_dom_sum/num_batches:<12.4f} | {val_cls:<10.4f} | {val_dls:<12.4f} | {t_mde:<8.4f} | {save_mark:<10}")
