...
# 2. 使用沒有缺失值的資料集
# 3. 標準的 CDAN 實作（如 tllib）通常會將輸入 Discriminator 的分類預測結果 detach (截斷梯度)。(怕干擾到分類任務學習，待觀察)
# 4. 修改最佳模型選擇策略: score = source acc - lambda x target entropy
# ================================================================
...
class DSBN1d(nn.Module):
    ...
    def forward(self, x, domain_label):
        ...
        if domain_label == 'source':
            ...
            raise ValueError("domain_label must be 'source' or 'target'")

class FeatureExtractorWithDSBN(nn.Module):
    def __init__(self, input_dim, hidden_dim=64):
        super(FeatureExtractorWithDSBN, self).__init__()
        self.fc1 = nn.Linear(input_dim, 32)
        self.dsbn1 = DSBN1d(32)         # 替換掉原本的 BatchNorm1d
        self.relu1 = nn.ReLU(True)
        
        self.fc2 = nn.Linear(32, hidden_dim)
        self.dsbn2 = DSBN1d(hidden_dim) # 替換掉原本的 BatchNorm1d
        self.relu2 = nn.ReLU(True)

    def forward(self, x, domain_label):
        x = self.fc1(x)
        x = self.dsbn1(x, domain_label) # 告訴 DSBN 現在是哪一個 Domain
        x = self.relu1(x)
        
        x = self.fc2(x)
        x = self.dsbn2(x, domain_label) # 告訴 DSBN 現在是哪一個 Domain
        x = self.relu2(x)
        return x
...
class DualStreamCDAN(nn.Module):
    def __init__(self, num_aps=4, num_classes=49, hidden_dim=64, rtt_input_dim=1):
        ...
        # --- 使用內建 DSBN 的特徵提取器 ---
        self.rtt_extractor = FeatureExtractorWithDSBN(rtt_input_dim, hidden_dim)
        self.rssi_extractor = FeatureExtractorWithDSBN(6, hidden_dim)
    ...
    def forward(self, rssi, rtt, coord_tensor, alpha=1.0, domain_label='source'):
        """
        coord_tensor: 傳入所有 RP 的真實座標 (C, 2)，用於計算期望座標
        """
        f_rssi = self.rssi_extractor(rssi, domain_label) 
        f_rtt = self.rtt_extractor(rtt, domain_label)
...
def validate_process(model, source_val_loader, target_val_loader, device, coord_tensor):
    ...
    with torch.no_grad():
        ...
        for s_rssi, s_rtt, s_label in source_val_loader:
            ...
            class_out_s, _, _, _ = model(s_rssi, s_rtt, coord_tensor, alpha=0, domain_label='source')
        ...
        for t_rssi, t_rtt, _ in target_val_loader:
            ...
            _, _, _, softmax_t = model(t_rssi, t_rtt, coord_tensor, alpha=0, domain_label='target')
...
def evaluate_test(model, data_loader, coord_tensor, device, domain_label, return_all_errors=False):
    model.eval()
    ...
    with torch.no_grad():
        for rssi, rtt, labels in data_loader:
            ...
            class_out, _, _, _ = model(rssi, rtt, coord_tensor, alpha=0, domain_label=domain_label)
    ...
    for seed in seed_candidate:
        ...
        for epoch in range(num_epochs):
            ...
            for (s_rssi_b, s_rtt_b, s_lbl_b), (t_rssi_b, t_rtt_b, _) in zip(source_loader, target_train_loader):
                ...
                # 訓練階段
                cls_out_s, d_logits_rssi_s, d_logits_rtt_s, softmax_s = model(s_rssi_b, s_rtt_b, COORD_TENSOR, alpha=alpha, domain_label='source')
                _, d_logits_rssi_t, d_logits_rtt_t, softmax_t = model(t_rssi_b, t_rtt_b, COORD_TENSOR, alpha=alpha, domain_label='target')
                
                # 修改處：將 CE 替換為 Distance-Weighted CE
                # 只有當 Epoch 大於 10 (讓基礎分類器先學會基本特徵) 時，才加入距離懲罰
                current_alpha_dist = 0.1 if epoch > 10 else 0.0
                loss_cls = distance_weighted_ce(cls_out_s, s_lbl_b, COORD_TENSOR, alpha_dist=current_alpha_dist)
            ...
            if (epoch + 1) % 1 == 0:
                t_acc, t_mde, _ = evaluate_test(model, target_test_loader, COORD_TENSOR, DEVICE, domain_label='target')
        ...
        t_acc, t_mde, t_errors = evaluate_test(model, target_test_loader, COORD_TENSOR, DEVICE, domain_label='target', return_all_errors=True)
        s_acc, s_mde, s_errors = evaluate_test(model, source_test_loader, COORD_TENSOR, DEVICE, domain_label='source', return_all_errors=True)
