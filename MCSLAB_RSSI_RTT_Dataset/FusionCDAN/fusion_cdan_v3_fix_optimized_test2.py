...
class DualStreamCDAN(nn.Module):
    ...
    def forward(self, rssi, rtt, coord_tensor, alpha=1.0):
        ...
        # g_cond = torch.cat((softmax_output.detach(), expected_coords.detach()), dim=1)
        # Branch A
...
#     # 定義常數：避免與真實負值 (-1207 等) 衝突
#     # 建議設為 -10000 (代表 -10公尺，物理上絕對不可能的數值)
#     RSSI_MISSING_VALUE = -100.0
#     RTT_MISSING_VALUE = -10000.0 
...
#     # 1. 初始化無效值 (Pre-cleaning)
#     # 先把 CSV 裡可能已經存在的 error code (如 0, -1) 或極端值轉為 NaN
#     # 注意：這裡不能把負數全部轉 NaN，因為 -1207 是有效值！
#     # 我們只針對「等於 0」或「等於 -1」這種明顯的錯誤碼做處理
#     for col in rssi_cols:
#         # RSSI 不太可能剛好是 0 (通常是 -30 ~ -90)，0 通常是錯誤
#         df[col] = df[col].replace([0, -100], np.nan) 
    
#     for col in rtt_cols:
#         # RTT 距離 0 也是極端少見 (除非貼在天線上)，通常 0 代表失敗
#         # 這裡特別小心：只取代 0 和 -1，保留 -1207 這種真實雜訊
#         df[col] = df[col].replace([0, -1], np.nan)

#     # 2. 填補邏輯
#     if is_source:
#         # Source (Train): 優先用 Label 平均值填補
...
#     # Target (Test): 絕對不計算平均，避免洩漏
#     # 直接跳過，讓它落入下方的最終填補

#     # 3. 最終填補 (Final Imputation)
#     # 將所有剩餘的 NaN 填補為我們設定的「安全常數」
#     df[rssi_cols] = df[rssi_cols].fillna(RSSI_MISSING_VALUE)
#     df[rtt_cols] = df[rtt_cols].fillna(RTT_MISSING_VALUE)
...
#     # ==========================================
#     # 這裡原本的填補邏輯全部刪除！
#     # 因為你保證了 df 裡沒有 NaN，也沒有失敗代碼 (0 或 -1)
#     # ==========================================

#     # 2. 採樣 (Sampling) - 保持不變，為了類別平衡
#     if samples_per_label is not None:
#         df = df.groupby('Label').apply(
#             lambda x: x.sample(n=samples_per_label, replace=True) if len(x) < samples_per_label else x.sample(n=samples_per_label, replace=False)
#         ).reset_index(drop=True)
...
#     if is_source:
#         # Source (Train): 建立標準並轉換
#         rssi_data = rssi_scaler.fit_transform(rssi_data)
#         rtt_data = rtt_scaler.fit_transform(rtt_data)
#         labels = label_encoder.fit_transform(raw_labels)
#         is_scaler_fitted = True
#     else:
#         # Target (Test): 使用 Source 的標準轉換
#         if not is_scaler_fitted: raise ValueError("Error: Scaler not fitted.")
#         rssi_data = rssi_scaler.transform(rssi_data)
#         rtt_data = rtt_scaler.transform(rtt_data)
#         try: labels = label_encoder.transform(raw_labels)
...
def main():
    ...
    for seed in seed_candidate:
        ...
        for epoch in range(num_epochs):
            ...
            # total_target_entropy_sum = 0.0
            # total_target_samples = 0
            # total_rejected_samples = 0
            # ENTROPY_THRESHOLD = 3.0 # 設定你想觀察的門檻
            ...
            for (s_rssi_b, s_rtt_b, s_lbl_b), (t_rssi_b, t_rtt_b, _) in zip(source_loader, target_train_loader):
                ...
                # # --- 新增：統計 Target Entropy 資訊 (不影響原本的訓練流程) ---
                # batch_mean_entropy = entropy_t.mean().item()
                # batch_max_entropy = entropy_t.max().item()
                # batch_min_entropy = entropy_t.min().item()
                
                # # 計算這個 Batch 有多少樣本「高於」門檻 (也就是本來會被丟掉的數量)
                # rejected_count = (entropy_t >= ENTROPY_THRESHOLD).sum().item()
                # batch_size = entropy_t.size(0)
                
                # # 累加給 Epoch 統計用
                # total_target_entropy_sum += entropy_t.sum().item()
                # total_target_samples += batch_size
                # total_rejected_samples += rejected_count
                
                # # (可選) 每 50 個 batch 印出一次，避免洗版
                # if num_batches % 50 == 0:
                #     print(f"Batch {num_batches} | Target Entropy - Mean: {batch_mean_entropy:.3f}, Max: {batch_max_entropy:.3f}, Min: {batch_min_entropy:.3f} | Rejected: {rejected_count}/{batch_size}")
                # # -------------------------------------------------------------
                ...
                LAMBDA_DOM = 0.5
                loss = loss_cls + LAMBDA_DOM * (loss_dom_rssi + loss_dom_rtt)
            ...
            # # ----------------- 每個 Epoch 結束後印出總結 -----------------
            # epoch_avg_entropy = total_target_entropy_sum / total_target_samples
            # epoch_reject_ratio = (total_rejected_samples / total_target_samples) * 100

            # print(f"=== Epoch Summary ===")
            # print(f"Avg Target Entropy: {epoch_avg_entropy:.4f}")
            # print(f"Rejected Target Samples (>={ENTROPY_THRESHOLD}): {epoch_reject_ratio:.2f}% ({total_rejected_samples}/{total_target_samples})")
            # print(f"=====================")
